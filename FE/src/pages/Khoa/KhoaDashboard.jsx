...
const KhoaDashboard = () => {
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      // Xử lý logic quay lại bước trước
      if (currentStep === 3) {
        setCurrentStep(2);
      } else if (currentStep === 2) {
        setCurrentStep(1);
      }
      // Nếu đang ở bước 1, để trình duyệt xử lý mặc định (quay về trang trước)
    };

    // Thêm state vào history để có thể bắt sự kiện popstate
    if (currentStep > 1) {
  ...
  const handleXepHang = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Tự động chọn những sinh viên ĐỦ ĐIỀU KIỆN (xếp loại khác "KhongDuDieuKien" và mức HB > 0)
                      ...
                      <tr key={sv.maHoSo} className="bg-green-50/30">
                        ...
                        <td className="p-4 text-center">
                          <span className="font-bold text-purple-600">
                            {sv.diemHocTap != null
                              ? Number(sv.diemHocTap).toFixed(2)
                              : ""}
                          </span>
                          <span className="text-gray-400"> / </span>
                          <span className="font-bold text-green-600">
