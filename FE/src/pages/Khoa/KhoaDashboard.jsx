...
import {
    ClipboardList, PieChart, Users, Search, CheckCircle, XCircle, 
    Send, Filter, Award, Calculator, RefreshCw, AlertCircle, Loader
} from 'lucide-react';
...
const KhoaDashboard = () => {
    ...
    const handleXepHang = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                // Tự động chọn những sinh viên được nhận học bổng
    ...
    const handleChotDanhSach = async () => {
        ...
        if (!window.confirm(`Bạn có chắc chắn muốn chốt danh sách ${danhSachChon.length} sinh viên?`)) {
            return;
        }
        ...
        setError(null);
        try {
            const response = await khoaService.chotDanhSachDeXuat(maDot, danhSachChon);
            if (response.success) {
    ...
    const getXepLoaiColor = (xepLoai) => {
        switch (xepLoai) {
            case 'XuatSac': return 'bg-purple-100 text-purple-700';
            case 'Gioi': return 'bg-blue-100 text-blue-700';
            case 'Kha': return 'bg-green-100 text-green-700';
    ...
    const getXepLoaiText = (xepLoai) => {
        switch (xepLoai) {
            ...
            case 'Kha': return 'Khá';
            default: return 'Không đủ ĐK';
        }
    };

    // Filter danh sách
    const filteredList = currentStep === 1 
        ? hoSoChoDuyet.filter(hs =>
            hs.hoTenSinhVien.toLowerCase().includes(searchTerm.toLowerCase()) ||
            hs.maSV.includes(searchTerm)
        )
        : ketQuaXepHang?.danhSachXepHang.filter(sv =>
            ...
            <div className="flex justify-between items-end border-b border-gray-200 pb-5">
                ...
                <div className="flex gap-3">
                    {currentStep === 1 && (
                        ...
                        >
                            <Calculator size={18} /> Tiếp tục xếp hạng
                        </button>
                    )}
                    {currentStep === 2 && (
                        <button 
            ...
            {currentStep === 2 && ketQuaXepHang && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                        <p className="text-xs font-bold text-gray-400 uppercase">Đã chi tiêu</p>
                        <h4 className="text-2xl font-black text-gray-800 mt-2">
                            {ketQuaXepHang.tongChiTieu.toLocaleString('vi-VN')} đ
                        </h4>
                    </div>
                    <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                        <p className="text-xs font-bold text-gray-400 uppercase">Được nhận</p>
                        <h4 className="text-2xl font-black text-green-600 mt-2">
                            {ketQuaXepHang.soLuongDuocNhan} SV
                        </h4>
                    </div>
                    <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                        <p className="text-xs font-bold text-gray-400 uppercase">Tổng hồ sơ</p>
                        <h4 className="text-2xl font-black text-gray-800 mt-2">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-wrap gap-4 justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        ...
                        {currentStep === 1 ? 'Danh sách hồ sơ chờ duyệt' : 'Kết quả xếp hạng'}
                    </h3>
                    <div className="flex gap-3">
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        ...
                        // TASK 2.1: Bảng danh sách chờ duyệt
                        <table className="w-full text-left text-sm text-gray-600">
                            <thead className="bg-white border-b border-gray-200">
                                <tr>
                                    ...
                                    <th className="p-4 font-bold text-gray-700 text-center">GPA</th>
                                    <th className="p-4 font-bold text-gray-700 text-center">ĐRL</th>
                                    <th className="p-4 font-bold text-gray-700 text-center">Trạng thái</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredList.map((hs) => (
                                    <tr key={hs.maHoSo} className="hover:bg-blue-50/30 transition-colors">
                                        <td className="p-4 font-semibold text-gray-800">{hs.maHoSo}</td>
                                        ...
                                        <td className="p-4 text-center font-bold text-blue-600">{hs.gpa.toFixed(1)}</td>
                        ...
                        // TASK 2.2 & 2.3: Bảng kết quả xếp hạng
                        <table className="w-full text-left text-sm text-gray-600">
                            <thead className="bg-white border-b border-gray-200">
                                <tr>
                                    <th className="p-4 font-bold text-gray-700 text-center">Hạng</th>
                                    <th className="p-4 font-bold text-gray-700">MSSV</th>
                                    <th className="p-4 font-bold text-gray-700">Họ và Tên</th>
                                    <th className="p-4 font-bold text-gray-700 text-center">Lớp</th>
                                    ...
                                    <th className="p-4 font-bold text-gray-700 text-center">Mức HB</th>
                                    <th className="p-4 font-bold text-gray-700 text-center">Trạng thái</th>
                                    {currentStep === 2 && (
                                        <th className="p-4 font-bold text-gray-700 text-center">Chọn</th>
                                    )}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredList.map((sv) => (
                                    ...
                                    >
                                        ...
                                        <td className="p-4 font-semibold text-gray-800">{sv.maSV}</td>
                                        <td className="p-4">{sv.hoTen}</td>
                                        <td className="p-4 text-center">{sv.tenLop}</td>
                                        <td className="p-4 text-center">
                                            <span className="font-bold text-blue-600">{sv.gpa.toFixed(1)}</span>
                                            <span className="text-gray-400"> / </span>
                                            <span className="font-bold text-green-600">{sv.diemRenLuyen}</span>
                                        </td>
                                        <td className="p-4 text-center">
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${getXepLoaiColor(sv.xepLoai)}`}>
                                        ...
                                        {currentStep === 2 && (
                                            <td className="p-4 text-center">
                                                ...
                                                >
                                                    ...
                                                    {danhSachChon.includes(sv.maHoSo) ? 'Đã chọn' : 'Chọn'}
                                                </button>
                                            </td>
                                        )}
                                    </tr>
            ...
            {currentStep === 3 && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                        Danh sách đã được chuyển lên cấp Trường (Hội đồng) để xét duyệt.
                    </p>
                    <button
                        onClick={() => {
                            setCurrentStep(1);
